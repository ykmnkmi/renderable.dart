...
void main() {
  group('set', () {
    ...
    test('block', () {
      final template = envTrim.fromString('{% set foo %}42{% endset %}{{ foo }}');
    ...
    test('namespace redefined', () {
      final template = envTrim.fromString('{% set ns = namespace() %}{% set ns.bar = "hi" %}');
      expect(() => template.render({'namespace': () => {}}),
          throwsA(predicate((error) => error is TemplateRuntimeError && error.message == 'non-namespace object')));
    });

    test('namespace', () {
      final template = envTrim.fromString('{% set ns = namespace() %}{% set ns.bar = "42" %}{{ ns.bar }}');
      expect(template.render(), equals('42'));
    });

    test('namespace block', () {
      final template = envTrim.fromString('{% set ns = namespace() %}{% set ns.bar %}42{% endset %}{{ ns.bar }}');
      expect(template.render(), equals('42'));
    });

    test('init namespace', () {
      final template = envTrim.fromString('{% set ns = namespace(d, self=37) %}{% set ns.b = 42 %}{{ ns.a }}|{{ ns.self }}|{{ ns.b }}');
      expect(
          template.render({
            'd': {'a': 13}
          }),
    ...
    test('namespace loop', () {
      final template = envTrim.fromString('{% set ns = namespace(found=false) %}{% for x in range(4) %}{% if x == v %}'
    ...
    test('block filtered', () {
      final template = envTrim.fromString('{% set foo | trim | length | string %} 42    {% endset %}{{ foo }}');
    ...
    test('block filtered set', () {
      ...
      final template = envTrim.fromString('{% set a = " xxx " %}{% set foo | myfilter(a) | trim | length | string %} '
